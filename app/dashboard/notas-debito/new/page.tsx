...
import { createDebitNote, getDebitableInvoices } from "@/actions/debit-notes";
import { debitNoteReasons } from "@/lib/constants";
...
export default function NuevaNotaDebitoPage() {
    const [state, formAction, isPending] = useActionState(createDebitNote, null);
    const [invoices, setInvoices] = useState<Invoice[]>([]);
    const [selectedInvoice, setSelectedInvoice] = useState<Invoice | null>(null);
    const [items, setItems] = useState<Item[]>([{ description: "", quantity: 1, price: 0 }]);

    useEffect(() => {
        getDebitableInvoices().then(setInvoices);
    }, []);
    ...
    return (
        <div className="max-w-4xl mx-auto space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-3xl font-bold tracking-tight">Nueva Nota de Débito</h2>
                    <p className="text-muted-foreground mt-1">
                        Crea una nota de débito para cargos adicionales
                    </p>
                </div>
                <Link href="/dashboard/notas-debito">
            ...
            <form action={formAction} className="space-y-6">
                ...
                <div className="rounded-xl border border-border bg-card p-6 shadow-sm space-y-4">
                    <h3 className="text-lg font-semibold">Motivo de la Nota de Débito</h3>
                ...
                {state?.success && (
                    <div className="p-4 bg-green-50 text-green-600 rounded-lg">
                        Nota de débito creada exitosamente
                    </div>
                )}

                <div className="flex gap-4">
                    <Button type="submit" disabled={isPending || !selectedInvoice}>
                        {isPending ? "Creando..." : "Crear Nota de Débito"}
                    </Button>
                    <Link href="/dashboard/notas-debito">
