...
function generateControlNumber(
    ...
    const prefix = "DTE";
    const tipoDte = "06"; // Nota de Débito
...
export async function getDebitNotes(options?: {
    page?: number;
    limit?: number;
    status?: string;
}) {
    ...
    if (!session?.user?.id) {
        return { debitNotes: [], total: 0, totalPages: 0 };
    }
...
export async function getDebitableInvoices() {
    const session = await auth();
    if (!session?.user?.id) {
        return [];
    }
...
export async function createDebitNote(
    ...
    if (invoice.transmissionStatus !== "SENT") {
        return { success: false, error: "Solo se pueden crear notas de débito para facturas transmitidas" };
    }
    ...
    revalidatePath("/dashboard/notas-debito");
    return { success: true, debitNoteId: debitNote.id };
}

export async function generateDebitNoteDTE(debitNoteId: string) {
    const session = await auth();
    if (!session?.user?.id) {
    ...
    const debitNote = await prisma.debitNote.findFirst({
        where: { id: debitNoteId, userId: session.user.id },
        include: {
            items: true,
            invoice: {
                include: {
    ...
    const invoice = debitNote.invoice;
    ...
    const isCCF = debitNote.type === "CREDITO_FISCAL";
    ...
    // Obtener secuencia
    const ndCount = await prisma.debitNote.count({
        where: { userId: session.user.id, controlNumber: { not: null } },
    });
    const controlNumber = generateControlNumber(ndCount + 1, codEstable, codPuntoVenta);

    // Construir cuerpo del documento
    const cuerpoDocumento = debitNote.items.map((item, idx) => {
...
export async function getDebitNoteById(id: string) {
    ...
    return {
        ...debitNote,
        amount: Number(debitNote.amount),
